...
//ChromeDriverManager.getInstance().version("2.35").arch64().setup();//TODO
  //ChromeDriverManager.getInstance().setup("2.35");
  ...
  public static WebElement waitElement(By by, int... iSecToWait) {
    ...
    try {
      we = (new WebDriverWait(get(), iSecToWaitLoc)).
          until(visibilityOfElementLocated(by));//if this invoked from waitAndClick we will wait 2 sec by def
    } catch (Exception e) {
    }
    //===
    long timeElapsed = (System.currentTimeMillis() - timeStart) / 1000;
    ...
    try {
      ...
      try {
        sMsgTxtActual = (new WebDriverWait(get(), 3)).
            until(visibilityOfElementLocated(By.xpath(sXPath_FlashMsg))).getText();
      } catch (Exception e) {
      }
